...
class mernshell implements MernshellConfig {
  // Initial Values
  #auth: boolean = false;
  #base: boolean = false;
  #views: boolean = false;
  ...
  get template(): MernShellTemplate {
    if (this.fs) {
      return "fullstack";
    }
...
const mernshellConfig = new mernshell(flags, inputs);

export default mernshellConfig;
